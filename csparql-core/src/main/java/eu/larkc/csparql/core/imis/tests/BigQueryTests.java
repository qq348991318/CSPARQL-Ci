package eu.larkc.csparql.core.imis.tests;
...
//	/Users/yangfanyu/IdeaProjects/axonDB/mapdb 0

	public static void query(String path, String newPath) {
...
//		 		ECSTree queryTree = ecsq.getEcsTree();
				...
				for(BigQueryPattern key : queryAnswerListSet2.keySet()){
					for(ArrayList<BigECSTuple> dataPattern : queryAnswerListSet2.get(key)){
						for(eu.larkc.csparql.core.imis.models.BigECSTuple BigECSTuple : dataPattern){
				...
				for(eu.larkc.csparql.core.imis.models.BigQueryPattern BigQueryPattern : queryAnswerListSet2.keySet()){
					int count = 0;
				...
				for(int i = 0; i < 10; i++){
					...
					for(eu.larkc.csparql.core.imis.models.BigQueryPattern BigQueryPattern : f){
	...
	static public boolean findDataPatterns(BigExtendedCharacteristicSet ecs,
		...
		if(!links.containsKey(ecs)){
			...
			if(queryAnswerListSet2.containsKey(new BigQueryPattern(originalQueryLinks)))
				...
				HashSet<ArrayList<eu.larkc.csparql.core.imis.models.BigECSTuple>> d = new HashSet<ArrayList<eu.larkc.csparql.core.imis.models.BigECSTuple>>();
				d.add(list);
				queryAnswerListSet2.put(new BigQueryPattern(originalQueryLinks),d);
			}		
			...
			for(BigExtendedCharacteristicSet child : links.get(ecs)){
					...
					if(queryLinks.size()>1){
						...
						ArrayList<eu.larkc.csparql.core.imis.models.BigECSTuple> dummy = new ArrayList<eu.larkc.csparql.core.imis.models.BigECSTuple>();
						dummy.addAll(list);
